...
public:
  ...
  // RK4Compensator 使用
  double rk4_time_step = 0.003;
  int rk4_max_steps = 8000;
  double rk4_hit_distance_threshold = 0.02;
...
// CeresCompensator solves the linear-resistance model with ceres
class CeresCompensator : public TrajectoryCompensator {
public:
...
public:
  RK4Compensator() = default;

  double getFlyingTime(const Eigen::Vector3d &target_position) const noexcept override;
  std::vector<Eigen::Vector3d> sampleTrajectory(double pitch, double yaw, double speed,
                                                double t_total, std::size_t steps) const;
  Eigen::Vector3d simulate(double pitch, double yaw, double speed, double t_total) const;
...
public:
  static std::unique_ptr<TrajectoryCompensator> createCompensator(const std::string &type) {
    if (type == "ideal") {
      ...
      return std::make_unique<CeresCompensator>();
    } else if (type == "rk4") {
      return std::make_unique<RK4Compensator>();
    } else {
      return std::make_unique<IdealCompensator>();
    }
  }
