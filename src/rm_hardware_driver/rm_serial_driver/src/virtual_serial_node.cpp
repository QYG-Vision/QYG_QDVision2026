...
public:
  explicit VirtualSerialNode(const rclcpp::NodeOptions &options) : Node("serial_driver", options),state_machine_(qd::utils::VisionStateMachine::getInstance()) {
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(1), [this]() {
      rm_interfaces::msg::SerialReceiveData serial_receive_data_msg;
      serial_receive_data_msg.header.stamp = this->now();
      serial_receive_data_msg.header.frame_id = "odom";
      ...
      serial_receive_data_msg.bullet_speed = 25.0;
      serial_receive_data_msg.roll = roll;
      serial_receive_data_msg.pitch = pitch;
      serial_receive_data_msg.yaw = yaw;
      tf2::Quaternion q;
      ...
      serial_receive_data_pub_->publish(
        std::make_unique<rm_interfaces::msg::SerialReceiveData>(std::move(serial_receive_data_msg))
      );
      ...
      state_machine_.setVisionMode(static_cast<VisionMode>(mode)); // 状态机设置识别模式
...
RCLCPP_COMPONENTS_REGISTER_NODE(qd::serial_driver::VirtualSerialNode)
